...
RUN SQLX_OFFLINE=true cargo build --release --bin idempotent-ledger-rs
RUN rm -rf src
...
COPY --from=builder /app/target/release/idempotent-ledger-rs ./idempotent-ledger-rs
COPY --from=builder /app/migrations ./migrations

EXPOSE 3000

# distroless has no shell so CMD must use exec form
CMD ["./idempotent-ledger-rs"]
